...
def data_pre_preprocess(image):
    img = cv2.imread(image)
    img = img.astype(np.float32) / 255.
    img_lq = img2tensor(img, bgr2rgb=True, float32=True)
    return img_lq



class RGTModel:
    def __init__(self, name:str, upscale:int, device:str, model_path:str):
        self.name = None
        self.upscale = None
        self. v = None
        self.device = None
    ...
    def load_model(self, name, upscale, device, model_path):
        if self.name == name and self.upscale == upscale and self.device == upscale:
            logger.debug(f"Nothing changed, continued use loaded model")
            return 
        

        # init model by config
        if name == "RGT":
            num_heads = depth = [6,6,6,6,6,6,6,6]
        elif name == "RGT_S":
            num_heads = depth = [6,6,6,6,6,6]
        else:
            raise ValueError(f"model name wrong, should be RGT or RGT-S, but your input is: {name}")
        ...
        logger.debug(f"checkpoints path:{model_path}")
        checkpoints = torch.load(model_path)
        self.net_g.load_state_dict(checkpoints["params"])
        self.net_g.to(torch.device(device))
        self.net_g.eval()
        


    def __call__(self, image: Union[str, torch.Tensor], use_chop: bool = True):
        if isinstance(image, str):
            image = data_pre_preprocess(image)
...
if __name__ == "__main__":
    model_path = "/root/code/ComfyUI/models/RGT/RGT/RGT_x2.pth"
    rgt_model = RGTModel(name="RGT", upscale=2, device="cuda:0", model_path=model_path)
    res = rgt_model(image="/root/code/RGT/datasets/single/test_4.png", use_chop=True)
    # index = 0
    # for name in ["RGT", "RGT_S"]:
    #     for upsacle in [2, 3, 4]:
    #         rgt_model = RGTModel(name=name, upscale=upsacle, device="cuda:0")
    #         res = rgt_model(image="/root/code/RGT/datasets/single/test_4.png", use_chop=True)
    #         cv2.imwrite(f"/root/code/RGT/datasets/single/res_{index}.png", res)
    #         index += 1
